...
void pos_bucket_sort(std::vector<int> &pos_vector_to_sort) {
    ...
    for (int i = 0; i < max_number_size; i++) {
        ...
        for (const std::vector<int> &bucket: pos_buckets) {
...
void neg_bucket_sort(std::vector<int> &neg_vector_to_sort) {
    ...
    for (int i = 0; i < max_number_size; i++) {
        ...
        for (const int &number : neg_vector_to_sort) {
            int factor = std::pow(10, i+1);
            int second_factor = std::pow(10, i);
            int focus = 9 - (number % factor - number % second_factor) / second_factor;  // (343 modulo 100 - 343 modulo 10) / 10 = 4
    ...
    // Convert the numbers back to negative numbers
    for (int &number : neg_vector_to_sort) {
        number = number * -1;
    }
}
...
int main() {
    std::vector<int> vector_to_sort = {4356, 5364, 531, 322, 411, 788, -444, -333, -22, -1};
