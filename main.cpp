...
std::vector<int> bucket_sort(std::vector<int>& vector_to_sort) {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        for (int number : vector_to_sort) {
            ...
            std::cout << "Number as string " << number_as_string[0] << std:: endl;  // debug print
            int integer = number_as_string[0] - '0';  // Converts char to int by subtracting ascii codes. Example 51 (3) - 48 (0) = 3

            bucket_array[integer].push_back(number);
        }
    }

    print_vector_vector(bucket_array);  // debug print
